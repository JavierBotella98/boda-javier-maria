...
export default function TimelinePage() {
  return (
    <div>
      ...
      <div className="mx-auto max-w-xl px-8 pb-16 sm:px-6">
        <FadeIn>
          <ol className="relative border-s border-cream-dark ps-10">
            {timeline.map((item) => (
              <li key={item.event} className="mb-8 last:mb-0">
                <span
                  aria-hidden
                  className="absolute -start-3 flex h-8 w-8 items-center justify-center rounded-full bg-cream-dark text-base"
                >
                  {item.icon}
                </span>
